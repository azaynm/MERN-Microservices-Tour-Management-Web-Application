                ...
                <div key={index} className="card m-3" style={{ width: '200px', backgroundColor: '#FFEFCD' }}>
                    ...
                    <div className="card-body text-center">
                        ...
                        <button className="btn btn-secondary" style={{ backgroundColor: '#FFDB8B', borderColor: '#FFDB8B' }} onClick={() => {
                            if (method === 'add-food') {
                                navigate('/add-food', { state: { hotel } });
                            } else if (method === 'view-edit-food') {
                                navigate('/select-cuisine', { state: { hotel, method:"view-edit-food" } });
                            }
                            else if (method === 'add-activity') {
                                navigate('/add-activity', { state: { hotel } });
                            }
                            else if (method === 'view-edit-activity') {
                                navigate('/view-edit-activity', { state: { hotel, method:"view-edit-activity" } });
                            }
                            else if (method === 'view-activities') {
                                navigate('/view-activities', { state: { hotel } });
                            }
                        }}>
                            {method== "add-activity"|| method=='view-edit-activity'? 'Select Activity':'Select Hotel'}
